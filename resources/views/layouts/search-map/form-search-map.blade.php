<form action="{{url( app_get_locale(false,false,'/').config('property.property_route_prefix') )}}" class="form bravo_form d-flex justify-content-start" method="get" onsubmit="return false;">
    ...
    @if(!empty($property_map_search_fields))
        @foreach($property_map_search_fields as $field)
            @switch($field['field'])
                @case ('location')
                    @include('frontend.layouts.search-map.fields.location')
                @break
                @case ('attr')
                    @include('frontend.layouts.search-map.fields.attr')
                @break
                @case ('date')
                    @include('frontend.layouts.search-map.fields.date')
                @break
                @case ('price')
                    @include('frontend.layouts.search-map.fields.price')
                @break
                @case ('advance')
