    ...
    <div class="container-fluid">
        <div class="d-flex justify-content-between mb20">
            ...
            <div class="title-actions">
                <a href="{{ route('dashboard.news.create')}}" class="btn btn-primary">{{__("Add new Post")}}</a>
            </div>
        </div>
        ...
        <div class="filter-div d-flex justify-content-between ">
            <div class="col-left">
                @if(!empty($rows))
                    <form method="post" action="{{url('dashboard.news.bulkEdit')}}"
            ...
            <div class="col-left">
                <form method="get" action="{{url('/dashboard/news/')}} " class="filter-form filter-form-right d-flex justify-content-end flex-column flex-sm-row" role="search">
                                ...
                                @if($rows->total() > 0)
                                    @foreach($rows as $row)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route('dashboard.news.edit',['id'=>$row->id])}}" class="btn btn-primary btn-sm"><i class="fa fa-edit"></i> {{__('Edit')}}</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @else
