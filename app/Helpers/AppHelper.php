...
function setting_item($item,$default = '',$isArray = false){
 ...
 function getDashboard(){
    ...
    if( $user->rolez_id == 1){ // Admin

        return 'dashboard.home';

    }elseif($user->rolez_id == 2){ // Agency

        return 'vendor.dashboard';

    }elseif($user->rolez_id == 3){ // Agent

        return 'agent.dashboard';

    }elseif( $user->rolez_id == 4){ // User
...
{
    $options['walker'] = $options['walker'] ?? '\\App\\Walkers\\MenuWalker';
    ...
    if(!empty($setting))
    {
        foreach($setting as $l=>$menuId){
            if($l == $location and $menuId){
                $menu = (new \App\Models\Menu())->findById($menuId);
...
function set_active_menu($item){
    \App\Walkers\MenuWalker::setCurrentMenuItem($item);
}
...
function get_file_url($file_id,$size="thumb",$resize = true){
    ...
    return \App\Helpers\FileHelper::url($file_id,$size,$resize);
}
...
function add_action($hook, $callback, $priority = 20, $arguments = 1){
    $args = func_get_args();
    $m = \App\Helpers\HookManager::inst();
...
function get_reviewable_services(){
    ...
    if(!empty($custom_modules)){
        ...
        foreach($custom_modules as $module){
            ...
            {
                $services = call_user_func([$module,'getReviewableServices']);
